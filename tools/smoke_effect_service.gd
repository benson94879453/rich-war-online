...
func _run_smoke_check() -> void:
	var service: Variant = EffectServiceScript.new()
	...
	var money_result: Variant = service.apply_tile_effect(player, money_tile)
	...
	_expect_equal(money_result.source_type, EffectServiceScript.SOURCE_TILE, "money tile records tile source type")
	...
	var safe_result: Variant = service.apply_tile_effect(player, safe_tile)
	...
	var unsupported_result: Variant = service.apply_effect(&"unsupported_effect")
